    ...
    if len(response_bytes) >= 21:
        try:
            ...
            if "rws".find(cmdType) > -1 and response[-1] == "!":
                ...
                print(
                    f"{cmd} register = {register}, Data = {data}, TimeStamp = {round(timestamp, 2)}"
                )
...
def display_qsb_count_response(response_bytes):
    ...
    try:
        ...
        if (
            ...
            # only print and store if the count has changed
            if data != last_count:
                print(
                    f"Count = {data}, TimeStamp = {round(timestamp, 2)}   ",
                    end="\r",
                    flush=True,
                )

                streamed_data.append({"count": data, "timestamp": round(timestamp, 2)})
                last_count = data  # update last_count
        else:
            print(f"[Ignored] {response}", end="\r\n", flush=True)
    except Exception as ex:
        print(f"[Parse exception:] {ex} | Raw:{response_bytes}")


@atexit.register
def save_json_data():
    filename = f"qsb_data_{int(time.time())}.json"
    try:
        with open(filename, "w") as f:
            json.dump(streamed_data, f, indent=2)
        print(f"\nSaved {len(streamed_data)} entries to {filename}")
